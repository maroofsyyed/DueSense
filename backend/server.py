...
def _validate_environment():
    ...
    if not supabase_url:
        logger.error("SUPABASE_URL not set - database will not work")
    else:
        logger.info(f"   Supabase URL: {supabase_url[:40]}...")

    supabase_key = os.environ.get("SUPABASE_SERVICE_ROLE_KEY") or os.environ.get("SUPABASE_ANON_KEY")
    if not supabase_key:
        logger.error("SUPABASE_SERVICE_ROLE_KEY not set - database will not work")
    else:
        logger.info(f"   Supabase key: {supabase_key[:12]}...")
